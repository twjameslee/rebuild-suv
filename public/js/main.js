...
$(function() {
    ...
    hello.init(CLIENT_IDS, {
        redirect_uri: 'oauth2callback.html',
        //response_type:'code',
        scope:'email'
    });
    ...
    hello.on('auth.login', function(auth) {
        ...
        hello(auth.network).api('/me').then(function(r) {
            _user.email = r.email;
            _user.sid = _user.email.split("@")[0];
            _user.domain = r.domain;
            _user.thumbnailUrl = r.thumbnail;
            $('#profile_google').html('<img src="' + r.thumbnail + '" /> Hey ' + r.displayName + ' ' + r.email + ' domain: ' + r.domain);
            var g = hello(auth.network).getAuthResponse();
            console.log(_user);
            console.log(g);
        });
    });
    ...
    if (sub_title) $('.sub-title').html(sub_title);
